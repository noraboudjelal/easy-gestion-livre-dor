...
export default function TrophyExperience({ slug, adminMode = false }) {
  ...
  useEffect(() => { if (event?.best_outfit_enabled && voter) rpc().catch(() => {}); }, [event?.best_outfit_enabled, voter, rpc]);
  ...
  useEffect(() => {
    ...
    fixOldLabels();
    const obs = new MutationObserver(fixOldLabels); obs.observe(document.body, { childList: true, subtree: true });
  ...
  async function vote() {
    ...
    setStatus("");
    try {
      await rpc("vote", { category: category.id, look: selected });
      setStatus("Vote enregistré ✓"); setSelected(null); setCategory(null);
    } catch (e) { setStatus(e.message || "Impossible d’enregistrer le vote."); }
  }

  function nextVoter() {
    const token = newToken();
    window.sessionStorage.setItem(`trophy-voter-${slug}`, token);
  ...
  if (!event?.best_outfit_enabled || !state) return null;
  const results = state.results || [];
  const currentResult = results[revealIndex];

  return <section id="trophees-vote" style={S.wrap}>
    <h2 style={S.title}>🏆 Les Trophées</h2>

    {adminMode && !state.closed && <div style={S.admin}>
      <strong>Réglage organisateur</strong>
      <label style={S.label}>Heure de révélation
        <input type="datetime-local" value={revealAtInput} onChange={(e) => setRevealAtInput(e.target.value)} style={S.input}/>
      </label>
      <div style={S.row}><button onClick={configure} style={S.secondary}>Enregistrer l’heure</button><button onClick={revealNow} style={S.danger}>Révéler maintenant</button></div>
    </div>}
    ...
    {state.closed && revealing && currentResult && <div style={S.reveal} key={currentResult.id}>
      <div style={S.confetti}>✨ 🎉 ✨ 🏆 ✨ 🎉 ✨</div><h2>{currentResult.label}</h2>
      {(currentResult.winners || []).length ? <div style={S.winners}>{currentResult.winners.map((w) => <div key={w.id} style={S.winner}><img src={w.photo_url} alt={w.name} style={S.winnerPhoto}/><strong>{w.name}</strong></div>)}</div> : <p>Aucun vote pour ce trophée.</p>}
      {(currentResult.winners || []).length > 1 && <p>Ex æquo 🏆</p>}
      {revealIndex < results.length - 1 ? <button onClick={() => setRevealIndex((i) => i + 1)} style={S.discover}>Découvrir le trophée suivant →</button> : <button onClick={() => setRevealing(false)} style={S.secondary}>Terminer</button>}
    </div>}
...
const S = {
  wrap:{maxWidth:900,margin:"22px auto",padding:20,borderRadius:26,background:"rgba(255,255,255,.94)",boxShadow:"0 12px 36px rgba(0,0,0,.12)",color:"#30242a",textAlign:"center"},
  title:{margin:"0 0 14px",fontSize:"clamp(1.6rem,5vw,2.4rem)"}, note:{margin:"10px 0",fontSize:14},
  categories:{display:"flex",gap:8,flexWrap:"wrap",justifyContent:"center",margin:"14px 0"}, category:{border:0,borderRadius:999,padding:"10px 14px",fontWeight:800,cursor:"pointer"},
  voteBox:{marginTop:16,paddingTop:12,borderTop:"1px solid #eadfe3"}, grid:{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:12,margin:"14px 0"},
  photoButton:{position:"relative",padding:0,border:0,borderRadius:18,overflow:"hidden",background:"#fff",cursor:"pointer"}, photo:{display:"block",width:"100%",aspectRatio:"1 / 1",objectFit:"cover"}, name:{display:"block",padding:8,fontWeight:800}, check:{position:"absolute",top:8,right:8,width:30,height:30,borderRadius:999,background:"#d4a72c",color:"white",fontWeight:900,lineHeight:"30px"},
