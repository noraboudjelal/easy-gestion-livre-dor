...
export default function TicketSettings({business,onSaved}){
  ...
  async function toggleQueue(){
    const next=!queueOpen;setBusy(true);setMessage('');
    try{
      const response=await fetch(`/api/admin/tickets/${business.id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({is_open:next})});
      const data=await response.json();if(!response.ok)throw new Error(data.error);
      setQueueOpen(next);setMessage(next?'File ouverte.':'File fermée.');onSaved?.();
    }catch(err){setMessage(err.message||'Modification de la file impossible.');}finally{setBusy(false);}
  }
  ...
  return <details open style={{marginTop:12,minWidth:0,maxWidth:520,fontSize:13,overflowWrap:'anywhere'}}>
    ...
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,flexWrap:'wrap',padding:12,marginBottom:12,border:'1px solid #E6DDCC',borderRadius:10,background:'#FAF7F2'}}>
      <strong style={{color:queueOpen?'#287A45':'#A43B32'}}>{queueOpen?'● File ouverte':'● File fermée'}</strong>
      <button type="button" disabled={busy||!business.is_active} onClick={toggleQueue} style={{...button,fontWeight:800,borderColor:queueOpen?'#D9AAA5':'#A8D1B4'}}>{busy?'Patientez…':queueOpen?'Fermer la file':'Ouvrir la file'}</button>
      {!business.is_active&&<span style={{width:'100%',fontSize:11,color:'#8A6F5B'}}>Le commerce doit être activé avant d’ouvrir la file.</span>}
    </div>
