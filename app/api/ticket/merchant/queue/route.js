...
export async function GET(request) {
  ...
  const [{ data: queue, error }, { count, error: countError }] = await Promise.all([
    admin.from("ticket_queues").select("id, is_open, current_number, last_issued_number, estimated_minutes_per_client").eq("business_id", business.id).single(),
...
export async function POST(request) {
  ...
  const business = await activeBusiness(request);
  if (!business) return unauthorized();
  const { action, isOpen, minutes } = await request.json();
  if (action === "estimate") {
