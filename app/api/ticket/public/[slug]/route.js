...
export async function GET(request, { params }) {
  ...
  try {
    ...
    const {data:business,error} = await admin.from('ticket_businesses')
      .select('id,name,slug,offers_url,public_screen_enabled').eq('slug',params.slug).eq('is_active',true).maybeSingle();
    ...
    const base = {business_id:business.id,business_name:business.name,slug:business.slug,offers_url:offers,public_screen_enabled:business.public_screen_enabled};
