...
const offers=[
  {id:'le-fil',number:'01',name:'Le Fil – Mariage',audience:'Pour votre mariage',price:'250 €',unit:'/ événement',intro:'Tous les moments de votre mariage, réunis dans une expérience à partager.',features:['Borne comprise dans la formule','Photos, vidéos, messages écrits et vocaux des invités','Quiz, sondages et animations selon votre configuration','Livre souvenir numérique imprimable'],note:'Vos invités participent sur la borne ou depuis leur téléphone, grâce à un QR code ou un lien.',cta:'Parlons de mon mariage'},
...
const demos=[
  ...
  {name:'Le parcours Lehnova Ticket',text:'Découvrez une vraie file de démonstration et ses promotions.',href:'/demo-ticket-boucherie',image:'/vitrine-commerce.png',alt:'Aperçu d’un catalogue de commerce Lehnova',label:'Ticket & offres'},
...
export default function Home(){return <main className={`${styles.page} ${display.variable} ${body.variable}`}>
  ...
  <section id='accessoires' className={`${styles.container} ${styles.options}`}><div><p className={styles.eyebrow}>Pour compléter votre solution</p><h2>Accessoires <em>& options</em></h2><p>Des supports et des contenus complémentaires, choisis selon votre usage. Ils accompagnent votre solution Lehnova.</p></div><div><div className={styles.chips}>{['Supports plexiglas','QR codes','Supports NFC','Impressions','Personnalisation','Supports mariage','Page ou catalogue numérique'].map(s=><span key={s}>{s}</span>)}</div><a className={styles.textLink} href='/catalogue-numerique-toulouse'>Découvrir l’option page / catalogue →</a><p className={styles.optionNote}>Choix, quantités et tarif des options précisés avec vous. Le livre souvenir numérique imprimable est présenté dans la formule Le Fil ; l’impression et les supports physiques sont à préciser séparément.</p></div></section>
  <section id='demonstrations' className={`${styles.container} ${styles.section}`}><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>Découvrez par vous-même</p><h2>Du concret,<br/><em>à portée de clic.</em></h2></div><p>Explorez les démonstrations existantes.<br/>Les contenus sont des exemples de configuration.</p></div><div className={styles.demoGrid}>{demos.map(d=><a href={d.href} key={d.href} className={styles.demo}><div className={styles.demoImage}><Image src={d.image} fill sizes='(max-width: 700px) 90vw, 33vw' alt={d.alt}/><span>{d.label||'Démonstration'}</span></div><div className={styles.demoText}><h3>{d.name} <span aria-hidden='true'>↗</span></h3><p>{d.text}</p></div></a>)}</div><a className={styles.textLink} href='/demo-boucherie'>Voir aussi le catalogue de démonstration boucherie →</a></section>
