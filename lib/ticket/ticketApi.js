...
async function withEstimatedMinutes(state) {
  ...
  const { data } = await requireSupabase().from("ticket_queues").select("estimated_minutes_per_client").eq("business_id", state.business_id).maybeSingle();
  return { ...state, estimated_minutes_per_client: data?.estimated_minutes_per_client ?? null };
}
