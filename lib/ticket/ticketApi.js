...
async function withEstimatedMinutes(state, slug) {
  if (!state?.business_id) return state;
  let data;
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 5000);
  try {
    const response = await fetch('/api/ticket/public/' + encodeURIComponent(slug) + '?queue=1', {cache:'no-store',signal:controller.signal});
    if(response.ok) data = await response.json();
  } catch {} finally { clearTimeout(timeout); }
  if(!data) {
    // A summary outage must not lose the recovered ticket.
    const result = await requireSupabase().from("ticket_queues").select("estimated_minutes_per_client, queue_mode, manual_waiting_count, public_wait_display_enabled").eq("business_id", state.business_id).maybeSingle();
    data = result.data;
  }
  const queueMode = data?.queue_mode || "tickets";
  const automaticCount = Math.max(0, (state.last_issued_number || 0) - (state.current_number || 0));
  return { ...state, estimated_minutes_per_client: data?.estimated_minutes_per_client ?? null, queue_mode: queueMode, public_screen_enabled: data?.public_screen_enabled !== false, public_wait_display_enabled: data?.public_wait_display_enabled !== false, public_waiting_count: queueMode === "manual" ? data?.manual_waiting_count || 0 : data?.waiting_count ?? automaticCount };
}

export async function getPublicTicketState(slug, resumeToken) {
  return withEstimatedMinutes(await rpc("ticket_public_state", { p_slug: slug, p_resume_token: resumeToken || null }), slug);
}

export async function takeOrResumeTicket(slug, resumeToken) {
  return withEstimatedMinutes(await rpc("ticket_take_or_resume", { p_slug: slug, p_resume_token: resumeToken }), slug);
}
...
export function subscribeToQueue(businessId, onChange) {
  ...
  const channel = client
    // Each mounted consumer owns its channel; the dashboard and its statistics
    // can coexist without adding handlers to an already subscribed channel.
    .channel(`ticket-queue-${businessId}-${++queueSubscriptionId}`)
    .on(
      "postgres_changes",
...
export async function signInMerchant(code, expectedMode = "tickets") {
  const response = await fetch("/api/ticket/merchant/session", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code, expectedMode }) });
  const data = await response.json();
